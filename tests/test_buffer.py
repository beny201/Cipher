import pytest
from functionality.buffer import *
...
def test_should_return_dic(set_object):
    ...
    buffer.append_text(text_object)
    assert buffer.data_for_saving() == [asdict(text_object), asdict(
        text_object)]
...
"""def test_raise_error_when_not_int(mocker):
    buffer = Buffer()
    text_object = Text()
    buffer.append_text(text_object)
    ...
    with pytest.raises(ValueError):
        buffer.user_action()
"""
# do sprawdzenia czego nie bangla